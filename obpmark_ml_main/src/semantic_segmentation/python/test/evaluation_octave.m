...
% Adapted to Octave in February 2022 by Jannis Wolf (wolf.jannis@bsc.es) 
% at Barcelona Supercomputing Center

%%
%clear
gt_folder_path = '../../data/38-Cloud_test/Entire_scene_gts';
preds_folder_root = '../../data/38-Cloud_test/';
preds_folder = 'gt_patches';
...
function  related_patches = get_patches_for_sceneid (result_root, preds_dir, sceneid)
    ...
    ps = ~cellfun('isempty',strfind({files_inside(3:end,1).name},sceneid))
...
thresh = 0.5; % last layer has sigmoid activation
...
for n = 1:length(all_uniq_sceneid)
    ...
    for pcount = 1:length(scid_related_patches)
        ...
        predicted_patch =  im2bw(predicted_patch,thresh);
